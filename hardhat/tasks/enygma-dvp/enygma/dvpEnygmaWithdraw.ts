...
task('dvp:enygma:withdraw', 'Withdraw Enygma token from Dvp')
  .addParam(
    'pn',
    'The Privacy Node identification (ex: A, B, C, D)'
  )
  .addParam('symbol', 'Symbol of the Enygma token')
  .addParam('amount', 'Human-readable amount to withdraw (e.g. "100" for 100 tokens)')
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      ...
      console.log(`Withdrawing ${taskArgs.amount} (${amount} base units) of token ${taskArgs.symbol} from Dvp`);

      const tx = await token.callWithdrawFromDvp(amount);
      ...
      console.error(`Error during withdraw:`, error instanceof Error ? error.message : error);
    }
