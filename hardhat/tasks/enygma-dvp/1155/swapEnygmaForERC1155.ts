...
task('dvp:enygma:erc1155:swap', 'Swap Enygma token for ERC1155 token')
    .addParam(
        'pn',
    ...
    .addParam('nftAmountOrOne', 'nftAmountOrOne')
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        try {
            ...
            console.log(`Swapping Enygma -> ERC1155 id=${taskArgs.nftId} with chain=${taskArgs.chainId} sharedId=${taskArgs.sharedId}`);
            const tx = await enygmaToken.swapWithDvpForERC1155(taskArgs.nftId, taskArgs.nftAmountOrOne, taskArgs.nftResourceId, taskArgs.amount, taskArgs.chainId, taskArgs.sharedId, 0, { gasLimit: 5000000 });
            await tx.wait(1);
        } catch (error) {
            console.error(error);
        } finally {
