...
task('participants:update-role', 'Update participant on the Private Network Hub')
  .addParam('chainid', 'ChainID of the participant')
  .addParam('role', 'New role of the participant')
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const newParticipantRole = taskArgs.role;
    ...
    let tx = await participantStorage.updateRole(newParticipantChainId, newParticipantRole);

    console.log(`Participant ${newParticipantChainId} status updated to ${newParticipantRole}`);
  });
