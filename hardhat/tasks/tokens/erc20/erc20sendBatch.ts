...
// npx hardhat sendBatchTokens --pn-origin A --pn-dest B --token MYTOKEN --destination-address 0xf9260c378ea6e428a79eafe443bd24ea09af8bc9 --amount 1 --total-transactions 1000
task('tokens:erc20:send-batch', 'Sends a token from one privacy node to the other one')
  ...
  .addParam('totalTransactions', 'The total tx to be sent over to the other PN')
  .setAction(async (taskArgs, hre) => {
    ...
    const resourceId = process.env[`TOKEN_${taskArgs.symbol}_RESOURCE_ID`] as string;
    ...
    const deploymentRegistryAddress = process.env[`PRIVACY_NODE_${taskArgs.pnOrigin}_DEPLOYMENT_PROXY_REGISTRY`] as string; 
