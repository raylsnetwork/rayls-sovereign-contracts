...
task('unfreeze-token', 'Unfreezes a token for a list of participants')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const tx = await TokenRegistry.unfreezeToken(resourceId, participants.split(',').map(Number));
    const receipt = await tx.wait();
    if (receipt?.status === 1) {
      console.log(`Token ${resourceId} unfrozen for participants ${participants}`);
    }
  });
