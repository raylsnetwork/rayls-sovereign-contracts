...
task('tokens:erc1155:send', 'Sends a token from one privacy node to the other one')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const token = await getTokenErc1155BySymbol(hre, taskArgs.pnOrigin, tokenSymbol);
    try {
      const tx = await token.teleportAtomic(taskArgs.destinationAddress as string, taskArgs.id, taskArgs.amount, destinationChainId, "0x");
