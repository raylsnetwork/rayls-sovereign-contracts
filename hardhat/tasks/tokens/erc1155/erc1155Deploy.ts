...
/**
 * Builds the deploy action, parameterized by the concrete ERC1155 contract to instantiate.
 * Production uses `ProductionErc1155Token` (canonical handler surface, matches the factory-seeded
 * template); the `_test` task uses `RaylsErc1155Example` (test-only hooks / preminting).
 */
function makeErc1155DeployAction(contractName: string) {
    return async (taskArgs: any, hre: any) => {
        ...
        const contracts = await getDeploymentProxyRegistryAddress(['Endpoint', 'RNEndpoint', 'RNUserGovernance'], deploymentRegistryAddress, signer, hre.ethers);

        const tokenPN = await token.connect(signer).deploy(taskArgs.uri, taskArgs.name, contracts[0], contracts[1], contracts[2], { gasLimit: 5000000 });
...
task("tokens:erc1155:deploy", "Deploys ERC1155 (ProductionErc1155Token) on the PN")
    .addParam("pn", "The Privacy Node identification (ex: A, B, C, D)")
    .addParam("uri", "The ERC 1155 URI")
    .addParam("name", "The name of the ERC 1155 contract")
    .setAction(makeErc1155DeployAction("ProductionErc1155Token"));

task("tokens:erc1155:deploy_test", "Deploys the test ERC1155 (RaylsErc1155Example) on the PN")
    .addParam("pn", "The Privacy Node identification (ex: A, B, C, D)")
    .addParam("uri", "The ERC 1155 URI")
    .addParam("name", "The name of the ERC 1155 contract")
    .setAction(makeErc1155DeployAction("RaylsErc1155Example"));
