...
Scope.task('deploy', 'Deploys a Rayls ERC1155 contract')
  ...
  .setAction(async (taskArgs: DeployArgs, hre) => {
    ...
    const tokenPN = await token
      .connect(signer)
      .deploy(
        'url',
        taskArgs.name,
        process.env[`PRIVACY_NODE_${taskArgs.pn}_ENDPOINT_ADDRESS`] as string,
        process.env[`PRIVACY_NODE_${taskArgs.pn}_RAYLS_NODE_ENDPOINT_ADDRESS`] as string,
        process.env[`PRIVACY_NODE_${taskArgs.pn}_RAYLS_NODE_USER_GOVERNANCE`] as string,
        { gasLimit: 5000000 },
      );
    ...
    console.log(
      `\$ npx hardhat checkTokenResourceId --pn ${
        taskArgs.pn
      } --token-address ${await tokenPN.getAddress()}`,
    );
  });
...
Scope.task(
  'balances',
  'Check the balance of an Erc1155 token by Id on a Privacy Node',
)
  .addParam(
    'pns',
    `The PN's separated by comma (example: "A,B")`,
    undefined,
    types.string,
  )
  ...
  .setAction(async (taskArgs: BalancesArgs, { ethers }) => {
    ...
    spinner.start();
    const pns = taskArgs.pns.split(',');
    for (let pn of pns) {
      const token = await getTokenBySymbol(ethers, pn, taskArgs.token);
      ...
      console.log(`Found Implemented on PN ${pn} at Address ${tokenAddress}`);
      if (taskArgs.addressToCheck)
        console.log(
          `  Balance of ${
            taskArgs.addressToCheck
          } on ${pn}: ${await token.balanceOf(
            taskArgs.addressToCheck,
            taskArgs.tokenId,
          )}`,
        );
...
Scope.task('send', 'Sends a token from one Privacy Node to the other one')
  .addParam('token', 'The token name')
  .addParam(
    'pnOrigin',
    'The origin PN (ex: A, B, C, D)',
    undefined,
    types.string,
  )
  .addParam(
    'pnDest',
    'The destination PN (ex: A, B, C, D)',
    undefined,
    types.string,
  )
  .addParam(
  ...
  .setAction(async (taskArgs: SendArgs, { ethers }) => {
    ...
    const destinationChainId = process.env[
      `PRIVACY_NODE_${taskArgs.pnDest}_CHAIN_ID`
    ] as string;
...
async function getTokenBySymbol(
  ...
  const endpoint = await ethers.getContractAt(
    'Endpoint',
    process.env[`PRIVACY_NODE_${pn}_ENDPOINT_ADDRESS`] as string,
    signer,
  );
