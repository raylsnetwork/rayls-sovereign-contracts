...
/**
 * @title Security Test: ERC1155 Access Control
 ...
 *
 * RELATED:
 * - Contract: RaylsErc1155Handler.sol
 */
contract UnlockAccessControlERC1155Test is Test, IERC1155Receiver {
    ...
    uint256 constant TOKEN_ID = 0; // Gold token from the example
    ...
    function setUp() public {
        ...
        // Deploy ERC1155 token with the mock endpoint
    ...
    function test_unlock_hasAccessControl() public {
        ...
        token.unlock(recipient, TOKEN_ID, AMOUNT, "");
    }
    ...
    function test_receiveTeleport_hasAccessControl() public {
        vm.prank(attacker);
        vm.expectRevert(abi.encodeWithSelector(RaylsAccessManaged.RaylsAccessManaged__Unauthorized.selector, attacker));
        token.receiveTeleport(attacker, TOKEN_ID, AMOUNT, "");
    }
    ...
    function test_receiveTeleportAtomic_hasAccessControl() public {
        vm.prank(attacker);
        // RaylsErc1155Example overrides receiveTeleportAtomic with receiveMethod modifier
        vm.expectRevert(abi.encodeWithSelector(RaylsAccessManaged.RaylsAccessManaged__Unauthorized.selector, attacker));
        token.receiveTeleportAtomic(recipient, TOKEN_ID, AMOUNT, "");
    }
    ...
    function test_revertTeleportMint_hasAccessControl() public {
        vm.prank(attacker);
        vm.expectRevert(abi.encodeWithSelector(RaylsAccessManaged.RaylsAccessManaged__Unauthorized.selector, attacker));
        token.revertTeleportMint(recipient, TOKEN_ID, AMOUNT, "");
    }
    ...
    function test_revertTeleportBurn_hasAccessControl() public {
        vm.prank(attacker);
        vm.expectRevert(abi.encodeWithSelector(RaylsAccessManaged.RaylsAccessManaged__Unauthorized.selector, attacker));
        token.revertTeleportBurn(recipient, TOKEN_ID, AMOUNT);
    }
