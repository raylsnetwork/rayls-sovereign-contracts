...
library MessageLib {
     ...
     * @param caller Address of the calling contract (for uniqueness across sender types)
     * @param fromChainId ID of the chain that dispatched the message
     * @param from Address that dispatched the message
     * @param toChainId ID of the chain that will receive the message
     ...
     */
    function computeMessageId(address caller, uint256 fromChainId, address from, uint256 toChainId, address to, bytes memory data)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encode(caller, fromChainId, from, toChainId, to, data));
    }
    ...
    function computeMessageBatchId(
        ...
        return keccak256(abi.encode(caller, fromChainId, from, messages));
    }
