...
interface IRaylsEndpoint {
    ...
    /**
     * @notice Sends a Rayls message to the specified address at a Rayls endpoint.
     *         Use this method for single message transactions with explicit metadata.
     * @param _dstChainId The destination chain identifier.
     * @param _destination The address on the destination chain.
     * @param _payload The verified payload to send to the destination contract.
     * @param transferMetadata The metadata for the transfer.
     */
    function send(uint256 _dstChainId, address _destination, bytes calldata _payload, BridgedTransferMetadata memory transferMetadata) external payable returns (bytes32 messageId);
    ...
    /**
     * @notice Retrieves the immutable private hub id .
     *         This identifier is unique in the context of multi-chain interactions.
     * @return The private hub ID.
     */
    function getPrivateHubId() external view returns (uint256);

    /**
     * @notice Register a contract address from private hub to be queried internally by the contracts
     * @param _contractName The name of the contract
     * @param _contractAddressOnPrivateHub The name of the contract
     */
    ...
    /**
     * @notice Triggers the protocol Rayls View key update.
     */
    ...
    /**
     * @notice Returns the AUTH-V3 access manager (authority) address for this endpoint
     * @return The RaylsAccessManagerV1 proxy address
     */
