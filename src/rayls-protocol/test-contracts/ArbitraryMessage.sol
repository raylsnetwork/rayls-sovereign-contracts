...
contract ArbitraryMessage is RaylsApp {
    ...
    BridgedTransferMetadata private metadata;
    ...
    constructor(bytes32 _resourceId, address _endpoint, address _raylsNodeEndpoint) RaylsApp(_endpoint, _raylsNodeEndpoint, address(0)) {
        ...
        metadata.assetType = RaylsBridgeableERC.CUSTOM;
        metadata.from = msg.sender;
    }

    function send1Message(string memory message, uint256 destChainId) public {
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveMsgA.selector, message), '', '', '', metadata);
    }

    function send1IncreaseCount(uint256 destChainId) public {
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveIncreaseCount.selector), '', '', '', metadata);
    }
    ...
    function send3Messages(string memory messageB, string memory messageC, string memory messageD, uint256 destChainId) public {
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveMsgB.selector, messageB), '', '', '', metadata);
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveMsgC.selector, messageC), '', '', '', metadata);
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveMsgD.selector, messageD), '', '', '', metadata);
    }
    ...
    function send3MessagesToDifferentChainIds(string memory messageB, string memory messageC, string memory messageD, uint256 destChainIdB, uint256 destChainIdC, uint256 destChainIdD) public {
        _raylsSendToResourceId(destChainIdB, resourceId, abi.encodeWithSelector(this.receiveMsgB.selector, messageB), '', '', '', metadata);
        _raylsSendToResourceId(destChainIdC, resourceId, abi.encodeWithSelector(this.receiveMsgC.selector, messageC), '', '', '', metadata);
        _raylsSendToResourceId(destChainIdD, resourceId, abi.encodeWithSelector(this.receiveMsgD.selector, messageD), '', '', '', metadata);
    }
    ...
    function send1MessageWithMetadata(string calldata message, uint256 destChainId, address toContract) public {
        metadata.to = toContract;
        _raylsSendToResourceId(destChainId, resourceId, abi.encodeWithSelector(this.receiveMsgA.selector, message), '', '', '', metadata);
    }
